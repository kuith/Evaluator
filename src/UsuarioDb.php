<?php
	...
	Class UsuarioDb{
		...
		public function buscarUsuarioPorNombre($nombreUsuario){
			$result = $this->dbcon->query("SELECT FROM usuario WHERE nombre = $nombreUsuario");
			return $rsult;
		}
		
		//buscar usuario por contraseña
		public function buscarUsuarioPorPassword($password){
			$result = $this->dbcon->query("SELECT FROM usuario WHERE password = $password");
			return $rsult;
		}
		
		//buscar usaurio por nombre o por contraseña
		public function buscarUsuarioPorNombreOPassword($nombre, $password){
			$result = $this->dbcon->query("SELECT FROM usuario WHERE nombre = $nombre OR password = $password");
			return $rsult;
		}
